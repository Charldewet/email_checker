#!/usr/bin/env python3
"""
Check Winterton Turnover Data
============================

This script uses the database API endpoints to query turnover data for TLC Winterton pharmacy.
"""
...
def check_winterton_turnover_direct(db):
    """Query Winterton turnover data directly from database"""
    print("\n" + "="*60)
    print("🏪 WINTERTON TURNOVER DATA - DIRECT DATABASE QUERY")
    print("="*60)
    ...
    # Get pharmacy ID for TLC WINTERTON
    pharmacy_id = db.get_pharmacy_id_by_code('TLC WINTERTON')
    if not pharmacy_id:
        print("❌ TLC WINTERTON pharmacy not found in database")
        return
    
    print(f"🏪 Pharmacy ID for TLC WINTERTON: {pharmacy_id}")
    ...
    try:
        ...
        if not results:
            print("❌ No turnover data found for TLC WINTERTON")
            return
...
def check_available_dates(db):
    """Check what dates have data available for Winterton"""
    print("\n" + "="*60)
    print("📅 AVAILABLE DATES FOR WINTERTON")
    print("="*60)
    ...
    try:
        ...
        if not results:
            print("❌ No data found for TLC WINTERTON")
            return
...
def main():
    """Main function to check Winterton turnover data"""
    print("🚀 Starting Winterton Turnover Check...")
    ...
    # Check turnover data
    check_winterton_turnover_direct(db)
    
    print("\n✅ Winterton turnover check completed!")
